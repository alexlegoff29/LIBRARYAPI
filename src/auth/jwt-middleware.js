...
exports.checkJwtTokenMiddleware = (req, res, next) => {
    ...
    if(!req.headers.authorization){
        return res.status(401).json({ message: 'Error: Missing or empty authorization header' });
    }
    ...
    if (!token) {
        return res.status(401).json({ message: 'Error: invalid authentication JWT' });
    }

    // -- Check jwt token is valid and provided by our API
    jwt.verify(token, JWT_SECRET, (err, decodedToken) => {
        if (err) {
            res.status(401).json({ message: 'Error: invalid or expired authentication JWT' });
        } else {
